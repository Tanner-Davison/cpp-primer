...
Sales_data::Sales_data(const std::string id, unsigned units_s, double price_p)
    : item_id(id), units_sold(units_s), price(price_p), revenue(price_p * units_s) {}
Sales_data::Sales_data(const std::string id, unsigned units_s) : Sales_data(id, units_s, 0) {};
...
#else
   // Linux/Unix uses localtime_r with parameters reversed

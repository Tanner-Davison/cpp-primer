...
int main(int argc, char* argv[]) {
   std::string input_path   = (argc > 1) ? argv[1] : "./input.txt";  // Compiler args defaults if none
   std::string input_person = (argc > 2) ? argv[2] : "./person.txt"; // Compiler args defaults if none
   ...
   if (read_person(inFile_person, person)) {
      std::cout << "---- Person Data Available ";

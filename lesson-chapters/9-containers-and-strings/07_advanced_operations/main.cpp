...
int main() {
  ...
  auto max_student =
      std::max_element(student_records.begin(), student_records.end(),
  ...
  std::cout << "   Vector and list are equal: " << (equal ? "true" : "false")
            << std::endl;
  ...
  bool vec1_less = std::lexicographical_compare(vec1.begin(), vec1.end(),
                                                vec2.begin(), vec2.end());
  std::cout << "   Vector1 < Vector2: " << (vec1_less ? "true" : "false")
            << std::endl;
  std::cout << "\n";

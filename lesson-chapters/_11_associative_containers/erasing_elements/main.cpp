...
void fill_map(auto& mapv, const auto& vec) {
   ...
   for (const auto& word : vec) {
      ...
      if (!success) {
         std::cout << "[!error] Failed to add " << word << " element to map!" << std::endl;
      } else {
         ++success_count;
      }
   }
   std::cout << "[fill_map]: \n\tSuccessfully filled map with: " << success_count << " elements!\n" << std::endl;
}

void remove_words_from_map(auto& map, const std::set<std::string>& remove_list) {
...
int main() {
   ...
   const std::vector<std::string> list_one{"one", "two",   "three", "four", "five",
                                           "six", "seven", "eight", "nine", "ten"};

   const std::set<std::string> remove_list{"eight", "nine"};
   fill_map(mapped_str, list_one);
   remove_words_from_map(mapped_str, remove_list);
